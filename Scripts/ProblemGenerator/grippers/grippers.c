...
// convert int to string
    ...
    sprintf(str, "%d", grobots);
    char *str2 = malloc(sizeof(char) * 10);
    sprintf(str2, "%d", grooms);
    char *str3 = malloc(sizeof(char) * 10);
    sprintf(str3, "%d", gobjects);
    char *str4 = malloc(sizeof(char) * 10);
    sprintf(str4, "%d", seed);

    char *file_name = malloc(sizeof(char) * 200);
    strcpy(file_name, "problems/gripper_");
    strcat(file_name, str);
    strcat(file_name, "_");
    strcat(file_name, str2);
    strcat(file_name, "_");
    strcat(file_name, str3);
    strcat(file_name, "_");
    strcat(file_name, str4);
    strcat(file_name, ".pddl");


  FILE *f = fopen(file_name, "w");
//   printf("Yes");
  fprintf(f, "(define (problem gripper_%d_%d_%d_%d)", grobots, grooms, gobjects, seed);
  fprintf(f, "\n(:domain gripper-strips)");
  fprintf(f, "\n(:objects ");
  for ( i = 0; i < grobots; i++ ) fprintf(f, "robot%d ", i + 1);
  fprintf(f, "- robot\n");
  for ( i = 0; i < grobots; i++ ) {
    fprintf(f, "rgripper%d ", i + 1);
    fprintf(f, "lgripper%d ", i + 1);
  }
  fprintf(f, "- gripper\n");
  for ( i = 0; i < grooms; i++ ) fprintf(f, "room%d ", i + 1);
  fprintf(f, "- room\n");
  for ( i = 0; i < gobjects; i++ ) fprintf(f, "ball%d ", i + 1);
  fprintf(f, "- object)");
  fprintf(f, "\n(:init");
  for ( i = 0; i < grobots; i++ ) {
    fprintf(f, "\n(at-robby robot%d room%d)", i + 1, 1+(int)(random() / (((double)RAND_MAX + 1)/ grooms)));
  ...
  for ( i = 0; i < gobjects; i++ ) fprintf(f, "\n(at ball%d room%d)", i + 1, 1+(int)(random() / (((double)RAND_MAX + 1)/ grooms)));
  fprintf(f, "\n)");
  fprintf(f, "\n(:goal");
  fprintf(f, "\n(and");
  for ( i = 0; i < gobjects; i++ ) fprintf(f, "\n(at ball%d room%d)", i + 1, 1+(int)(random() / (((double)RAND_MAX + 1)/ grooms)));
