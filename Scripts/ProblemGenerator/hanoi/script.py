...
def generate_state_combinations( list_of_states,block_number, number_of_files ):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 2:
    ...
    elif len(sys.argv) == 4 and sys.argv[1] == '--only':

        block = int(sys.argv[2])

        blocks_list = list([i for i in range(1,block)])
        pegs_list = list([-1,-2,-3])
        
        pegs_blocks_list = pegs_list + blocks_list
        ...
        for i in range(1000):
            ...
            state = generate_states(block, pegs_blocks_list)
                ...
                #     if state[idx] != -1 and state[idx] != -2 and state[idx] != -3:
                #         if idx+1 > state[idx]:
                #             flag_var = 1
        ...
        generate_state_combinations( states_dict[block],block, int(sys.argv[3]) )